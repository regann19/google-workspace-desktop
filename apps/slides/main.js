...
app.setAsDefaultProtocolClient('gslides')
...
function createWindow(url) {
  ...
  const win = new BrowserWindow({
    ...
    title: 'Google Slides', autoHideMenuBar: true, titleBarStyle: 'default',
    backgroundColor: '#1a1a1a',
    webPreferences: { contextIsolation: true, nodeIntegration: false, spellcheck: true, partition: 'persist:slides' }
  })

  win.webContents.session.setPermissionRequestHandler((wc, p, cb) => cb(true))
  win.loadURL(url || 'https://docs.google.com/presentation/u/0/')
...
app.on('open-file', (event, filePath) => {
  ...
  try {
    ...
    if (data.doc_id) {
      const url = `https://docs.google.com/presentation/d/${data.doc_id}/edit`
...
app.on('open-url', (event, url) => {
  event.preventDefault()
  const realUrl = url.replace('gslides://', '')
...
function createMenu() {
  const template = [
    { label: 'Google Slides', submenu: [
      { label: 'About Google Slides', role: 'about' }, { type: 'separator' },
    ...
    { label: 'Go', submenu: [
      { label: 'New Presentation', accelerator: 'Command+N', click: () => createWindow('https://docs.google.com/presentation/create') },
      { label: 'All Slides', accelerator: 'Command+1', click: () => { const w = BrowserWindow.getFocusedWindow(); if (w) w.loadURL('https://docs.google.com/presentation/u/0/') } }
    ]},
